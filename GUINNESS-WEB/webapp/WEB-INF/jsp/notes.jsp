			...
			guinness
					.ajax({
						...
						success : function(req) {
							...
							for (var i = 0; i < json.length; i++) {
								obj = json[i];
								document.querySelector('#commentListUl').innerHTML += "<li comment-id='"+obj.commentId+"'><img class='avatar' src='/img/avatar-default.png'>"
										+ obj.commentText + "    " + obj.createDate + " " + obj.userName + "</li>";
							}
						}
					});
		}
		...
		function addMember(userId, groupId) {
			...
			guinness.ajax({
				...
				success : function(req) {
					var json = JSON.parse(req.responseText)
					if (json === "unknownUser")
						guinness.util.alert("멤버추가 실패", "사용자를 찾을 수 없습니다!");
					else if (json === "joinedUser")
						guinness.util.alert("멤버추가 실패", "사용자가 이미 가입되어있습니다!");
					else {
						appendMember(json);
					}
